...
interface KPIs {
    ...
    TotalMPISent?: number;
    ClosedROsWithTechVideoMade_Percent?: number;
    InspectionsComnpletedSentToCustomer_Percent?: number;
}

const normalizeValue = (val: any): number | undefined => {
...
const formatReviewMarkdown = (dealerName: string, userName: string, kpis: KPIs | null, debugInfo?: string): string => {
    ...
    const getVal = (val: number | undefined, isPercent: boolean = false) => {
        if (val === undefined || val === null) return "No data returned";
        return isPercent ? `${val}%` : val;
    };

    const getInsight = (val: number | undefined, benchmark: number, label: string) => {
...
export const CoachService = {
    generateReview: async (vitallyUuid: string, dealerName: string, userName: string): Promise<string> => {
        try {
            ...
            // Mapping based on multiple possible conventions:
            const kpis: KPIs = {
                // Active Conversation
                ActiveConversation: normalizeValue(
                    source['Active Conversation %'] ||
                    source['vitally.custom.activeConversation'] ||
                    source['bigquery.ActiveConversation']
                ),
                // Total MPIs
                TotalMPISent: normalizeValue(
                    source['MPI Sent %'] ||
                    source['MPI Sent % '] ||
                    source['vitally.custom.mpiMade30Day'] ||
                    source['Total MPIs Sent']
                ),
                // Video Adoption
                ClosedROsWithTechVideoMade_Percent: normalizeValue(
                    source['Closed ROs TV Made %'] ||
                    source['vitally.custom.techVideoMade30Day'] ||
                    source['Closed ROs w/ Tech Video (%)']
                ),
                // Inspections Completed
                InspectionsComnpletedSentToCustomer_Percent: normalizeValue(
                    source['% of ROs w MPI Completed'] ||
                    source['MPI Completed to Made %'] ||
                    source['vitally.custom.mpiMade30Day'] || // Fallback if no specific % field
                    source['Inspections Completed (%)']
                )
            };
            ...
            return formatReviewMarkdown(dealerName, userName, null, `Technical error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
};
